...
function insertMember($nama, $nomor_member, $alamat, $tgl_mendaftar, $tgl_terakhir_bayar)
{
    $statement = koneksi()->prepare('INSERT INTO member (nama_member, nomor_member, alamat, tgl_mendaftar, tgl_terakhir_bayar) VALUES (:nama_member, :nomor_member, :alamat, :tgl_mendaftar, :tgl_terakhir_bayar)');
    $result = $statement->execute(array(':nama_member' => $nama, ':nomor_member' => $nomor_member, ':alamat' => $alamat, ':tgl_mendaftar' => $tgl_mendaftar, ':tgl_terakhir_bayar' => $tgl_terakhir_bayar));
    if (!empty($result)) {
...
{
    $statement = koneksi()->prepare('UPDATE member SET nama_member="'.$nama.'", nomor_member="'.$no_member.'", alamat="'.$almt.'", tgl_mendaftar="'.$tgl_daftar.'", tgl_terakhir_bayar="'.$tgl_terakhir_bayar.'"WHERE id_member='.$id);
    $result = $statement->execute();
    if ($result) {
        header('location:Member.php');
    }
